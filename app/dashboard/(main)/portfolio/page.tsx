...
// ─── Export helpers ────────────────────────────────────────────────────────────
function exportProjectsCSV(projects: Project[]) {
  const headers = "id,title,slug,type,year,client";
  const rows = projects.map((p) =>
    [p.id, `"${p.title}"`, p.slug, p.type, p.year, `"${p.client ?? ""}"`].join(",")
  );
...
function exportProjectsJSON(projects: Project[]) {
      ...
      <div className="flex items-start justify-between mb-10 gap-4">
        <div>
          <h1 className="font-display font-extrabold text-3xl tracking-tight mb-2" style={{ color: "var(--text-primary)" }}>
            {t("dash.portfolio.section")}
          </h1>
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>
            {t("dash.portfolio.subtitle")}
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap justify-end shrink-0">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium border transition-all hover:opacity-80 disabled:opacity-40"
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all hover:opacity-85"
            style={{ backgroundColor: "var(--accent)", color: "#0a0a0a" }}
          >
            <Plus size={14} />
            {t("dash.portfolio.add")}
          </Link>
        </div>
      </div>
      ...
      {loading ? (
        ...
        >
          <p className="font-display font-bold text-lg mb-2" style={{ color: "var(--text-primary)" }}>
            {t("dash.portfolio.empty.title")}
          </p>
          <p className="text-sm mb-6" style={{ color: "var(--text-muted)" }}>
            {t("dash.portfolio.empty.desc")}
          </p>
          <Link
            href="/dashboard/portfolio/new"
